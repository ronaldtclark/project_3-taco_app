...
class TacoContainer extends Component {
  ...
  deleteTaco = async (id, e) => {
    ...
    try {
      const deleteTaco = await fetch('https://localhost:8000/tacos' + id, {
        method: 'DELETE'
      });
  ...
  closeAndEdit = async (e) => {
    ...
    console.log('close and edit')
      try {
        const editTaco = await fetch('https://localhost:8000/tacos' + this.state.editTacoId, {
          method: 'PUT',
  ...
  upVote = async (taco, e) => {
    ...
    const upVotedTaco = await fetch('https://localhost:8000/tacos/' + taco._id + '/upvote', { 
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      }
  ...
  downVote = async (tacoId, e) => {
    ...
    const downVotedTaco = await fetch('https://localhost:8000/tacos/' + tacoId + '/downvote', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      }
